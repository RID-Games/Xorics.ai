...
_SELF_EDIT_GUIDE = (
    ...
    "5. NEVER replace a non-blank line of working code with a test probe to make the suite pass. "
"A test probe is anything that satisfies a test without preserving real behaviour — placeholders, "
"hard-coded stubs, `pass` / `...` / `TODO` / `# pragma: no cover`, commented-out bodies, hard-coded "
"return values (`return True`, `return []`, `return 0`, `return {\"ok\": True}`), or trivial test "
"scaffolding whose only purpose is to satisfy an assertion. Empty lines and pure comments are fine "
"to add or remove. A failing suite means EITHER the code OR the test is wrong — fix the right one "
"(fix the code when the code is wrong, fix the test when the test is wrong); never substitute a "
"passing probe for the real behaviour. If you gut a non-blank line into a probe, that is a "
"REGRESSION, not a fix — do not report WRITE VERIFIED; instead, fix the line properly and re-run the "
"suite.\n"
